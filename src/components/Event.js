...
export default function Event({match}) {
    ...
    let filterEvent = events.find((event) => event.slug === match.params.slug );
    return (
        <article className="uk-article">

            <h1 className="uk-article-title"><a className="uk-link-reset" href="">{filterEvent.title}</a></h1>
            <img src={process.env.PUBLIC_URL + filterEvent.image} />

            <p className="uk-text-lead">{filterEvent.body}</p>
            ...
            <hr/>
                <div>
                    ...
                    <p>Категория</p>
                    {filterEvent.categories.map(category => <p>{Translations.CATEGORIES[category]}</p>)}
                    <p>Цена</p>
                    <p>{filterEvent.price}</p>
                </div>
